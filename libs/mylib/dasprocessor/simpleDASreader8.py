...
def load_DAS_files(filepaths, chIndex=None,samples=None, sensitivitySelect=0,
    ...
    signalnd  = np.asarray(data[samples,:],dtype=np.float32)*scale
...
def save_to_DAS_file(signal,filepath=None,datatype='processed'):
    ...
    h5pydict.save(filepath,dict(fileVersion=fileVersion,header=header,data=np.asarray(signal,np.float32)))   
