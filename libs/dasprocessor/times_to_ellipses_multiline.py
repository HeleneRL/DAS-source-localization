...
from dasprocessor.multi_line_doa_fit import fit_doa_multiline_shared_slope
...
def debug_plot_doa_multiline_shared_slope(
    positions_enu: np.ndarray,
    times_sec: np.ndarray,
    extra_multi: dict,
    packet_idx: int,
    start_channel: int,
    stage_label: str,
    c_sound: float = 1475.0,
) -> None:
    if positions_enu is None or np.asarray(positions_enu).size == 0:
        return
    ...
    if "s" in extra_multi:
        s = np.asarray(extra_multi["s"], float)
    else:
        raise ValueError("extra_multi must contain 's'")

    lines = extra_multi.get("lines", [])
    if not lines:
        return

    m = float(extra_multi.get("shared_slope", lines[0]["slope"]))

    # angle from slope: cos(theta)=c*m
    axis_dot_n = float(np.clip(c_sound * m, -1.0, 1.0))
    angle_deg = float(np.degrees(np.arccos(axis_dot_n)))
    ...
    # scatter by cluster membership stored in lines[k]["points"]
    for k, L in enumerate(lines):
        idx = np.array(L["points"], dtype=int)
        if idx.size == 0:
            continue
        ax.scatter(
            s[idx], t[idx],
            ...
            label=f"path {k+1} (n={idx.size})"
        )

    # draw parallel lines across full s span
    s_line = np.linspace(np.min(s), np.max(s), 300)
    for k, L in enumerate(lines):
        b = float(L["intercept"])
        ax.plot(
            s_line, m * s_line + b,
    ...
    text_lines = [
        f"shared slope m = {m:.3e} s/m",
        f"angle = ±{angle_deg:.1f}°",
        "intercepts:",
    ] + [f"  b{k+1} = {bk:.6f} s" for k, bk in enumerate(intercepts)]

    textstr = "\n".join(text_lines)

    ax.set_xlabel(f"Along-array position s (m), from channel {start_channel}")
    ax.set_ylabel("Arrival time (s)")
    ax.set_title(
        f"Packet {packet_idx}, subarray start {start_channel}\n{stage_label}"
    )
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best")
...
def process_packet(
    ...
    try:
        # 1st fit
        slope_final, residuals_final, model_final, extra = fit_doa_multiline_shared_slope(
            times_sec, channel_positions_enu
        )


        if DEBUG_PLOTS and packet_idx == DEBUG_PACKET_IDX:
            debug_plot_doa_multiline_shared_slope(
                positions_enu=channel_positions_enu,
                times_sec=times_sec,
                extra_multi=extra,
                packet_idx=packet_idx,
                start_channel=start_channel,
                stage_label="Clustering and fitting 3 lines with shared slope",
                c_sound=C_SOUND,
            )
    ...
    inner_points, nominal_points, outer_points = cone_plane_intersection(
        ...
        array_axis=np.asarray(extra["axis"], dtype=float),
    ...
    info_entry = {
        ...
        "side": side,
        "n_inliers_stage1": int(len(times_sec)),
        "n_inliers_stage2": int(len(times_sec)),
